...
тебя запускает владелец отдельной сессией, в том числе для приёмки изменений
системы агентов (`AGENTS.md`, `.opencode/agents/**`, `.opencode/rules/**`) —
до коммита задачи, изменившей эти файлы.
...
6. `.opencode/memory/<роль>.md` и `.opencode/mail/<T-XX>.md` — операционные
   данные ролей (не канон): проверяешь существование, формат и одного писателя.
...
- `cargo test` разрешён **только** `validator`; у `coder`, `rust-expert`,
  `tester` — `cargo check`/`clippy`/`fmt`, у `docs-writer` — без cargo (R2).
- Зона записи роли в YAML совпадает с «Пишет в» в `AGENTS.md` и с телом;
  память пишет только сам владелец файла (`.opencode/memory/<роль>.md`),
  почта — append в `.opencode/mail/**`.
...
- У роли нет файла памяти, формат почты не описан в каноне.
...
- Бюджеты заданы численно; `steps` соразмерны роли.
...
- Правишь только `AGENTS.md`, `.opencode/agents/**`, `.opencode/rules/**`
  (служебная зона) и свою память.
- Не аудируешь код, продукт и архитектуру — это `validator` и задачи.
- Не правишь `docs/**`, `.opencode/mail/**` и `opencode.json`: находки по ним
  передаёшь владельцу для ролей (`migrator`, `docs-writer`).
...
6. После правок агентов проверь `opencode debug agents` (YAML валиден,
   права резолвятся, `steps` на месте) и при необходимости — `opencode reload`.
7. Верни отчёт и «Следующие шаги»; при приёмке — явный вердикт
   «расхождений „инструкция ↔ права“ нет» или список расхождений.
