...
## lead · 2026-09-26 · пакет подтверждён (права и очистка)

- Подтверждение (решение владельца, 2026-09-26): два коммита — права/инструкции
  и очистка логов — с push.
- Пакет:
  1. `chore: права ролей и норма техпроблем (по итогам проб)` — роли
     `validator`/`auditor`/`git`, правила `review.md`/`git-workflow.md`, отчёты
     адресных проверок `service-permissions-2026-09-26{,-r2}.md`, эта запись;
  2. `chore: очистка логов после сервисной операции` — `git` запускает
     `node .opencode/scripts/clean-logs.mjs` (сброс памяти, удаление ленты),
     затем коммит и push; подтверждение для коммита 2 — из истории
     (`git show HEAD:<лента>`).
- Дальше / риски: почта пуста, дерево чистое; пилот T-03 — отдельной сессией
  с веткой `feature/T-03-check-create`.
