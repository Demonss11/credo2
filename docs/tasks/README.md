...
противоречат поведению. Полный прогон `cargo test --all` выполняет только
`validator` (R2, [`../AGENTS.md`](../AGENTS.md) §Рабочая группа агентов);
остальные роли компилируют (`check`/`clippy`/`fmt`). При закрытии задачи:
обновить статус в карточке и в сводке, а также статус требования в
`../features/README.md`, если оно стало выполняться.
