...
- **Цикл агентов** (2026-09-26, T-11): Agile-петля `coder` → `rust-expert` →
  ...
  задача — [T-11](tasks/T-11-agent-cycle/README.md). Пилот завершён на T-03:
  цикл отработал, а правило H5 применено — приёмка r1 «принято с замечаниями» →
  правки → повторная проверка r2 «принято» (отчёты
...
- **`check.create` — обязательные `{name, source}` (T-03).** Инструмент принимает
  оба параметра (схема `required`, отсутствие/не-строка/пустая строка →
  `validation_failed`); `name` сверяется с заголовком `Правило {name}` (несовпадение
  называет оба имени); повторный вызов перезаписывает черновик («сохранить =
  обновить»). Ошибки `check.create` отдаются каноническим конвертом Q29/§4.5;
  прочие инструменты — до T-04. +14 тестов (юнит `src/mcp.rs` + stdio
  `tests/mcp_draft.rs`), DoD 92/92. Карточка —
  [`tasks/T-03-check-create/README.md`](tasks/T-03-check-create/README.md), отчёт
  приёмки — [`reviews/T-03-2026-09-26.md`](reviews/T-03-2026-09-26.md).
...
- **Ошибки `check.create` (T-03).** Ошибки валидации отдаются конвертом
  `{"error": {"code", "message"}}` с кодом `validation_failed` (было
  `{"error": "<строка>"}`) — ломающее для MCP-клиентов; клиенты, присылавшие
  только `source`, теперь получают `validation_failed` (оба параметра
  обязательны). Прочие инструменты сохраняют плоский конверт до T-04.
