...
- **Цикл агентов** (2026-09-26, T-11): Agile-петля `coder` → `rust-expert` →
  `tester` (добавляет тесты, не запускает) → `validator` (единственная роль с
  `cargo test` и полным DoD; возврат на доработку — в `coder`), затем
  `docs-writer` и `git`; лидер — переработанный `lead`. Введены память роли
  `.opencode/memory/<роль>.md` и почта задачи `.opencode/mail/<T-XX>.md`
  (чекпойнт до/после тяжёлых операций), `steps`-лимиты, а у `git` — пакетное
  подтверждение и идемпотентность. Решение — [D38](decisions/D38-agent-cycle.md),
